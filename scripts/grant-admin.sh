...
# Script to grant DEFAULT_ADMIN_ROLE to a new address
# Usage: ./scripts/grant-admin.sh <CONTRACT_ADDRESS> <NEW_ADMIN_ADDRESS> [RPC_URL]
...
# DEFAULT_ADMIN_ROLE is 0x00...00 (32 bytes of zeros)
...
echo -e "${CYAN}${BOLD}👑 New Admin:${NC} ${NEW_ADMIN_ADDRESS}"
...
HAS_ROLE=$(cast call "$CONTRACT_ADDRESS" "hasRole(bytes32,address)(bool)" "$DEFAULT_ADMIN_ROLE" "$NEW_ADMIN_ADDRESS" --rpc-url "$RPC_URL")

if [ "$HAS_ROLE" == "true" ]; then
    echo -e "${YELLOW}⚠️  Warning: ${NEW_ADMIN_ADDRESS} already has DEFAULT_ADMIN_ROLE${NC}"
    echo -e "${YELLOW}   Skipping grant operation.${NC}"
    exit 0
else
    echo -e "${GREEN}✓${NC} New admin does not have DEFAULT_ADMIN_ROLE (proceeding with grant)"
fi
echo ""

# Confirm before proceeding
echo -e "${BLUE}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${YELLOW}${BOLD}⚠️  WARNING: This will grant DEFAULT_ADMIN_ROLE to:${NC}"
echo -e "${YELLOW}   ${NEW_ADMIN_ADDRESS}${NC}"
echo ""
...
HAS_ROLE_AFTER=$(cast call "$CONTRACT_ADDRESS" "hasRole(bytes32,address)(bool)" "$DEFAULT_ADMIN_ROLE" "$NEW_ADMIN_ADDRESS" --rpc-url "$RPC_URL")
...
if [ "$HAS_ROLE_AFTER" == "true" ]; then
    ...
    echo "╚══════════════════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
    echo -e "${GREEN}✓${NC} ${NEW_ADMIN_ADDRESS} now has DEFAULT_ADMIN_ROLE"
    echo ""
    echo -e "${CYAN}${BOLD}📝 Next Steps:${NC}"
    echo -e "${YELLOW}   1)${NC} Verify on block explorer"
    echo -e "${YELLOW}   2)${NC} Optionally grant MINTER_ROLE and PAUSER_ROLE if needed"
    echo -e "${YELLOW}   3)${NC} Optionally revoke your own DEFAULT_ADMIN_ROLE if desired"
else
    echo -e "${RED}${BOLD}"
    echo "╔══════════════════════════════════════════════════════════════════════════╗"
    echo "║"
